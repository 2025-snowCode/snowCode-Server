...
public class ChatRoomController {
    ...
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "채팅방 목록 조회에 성공하였습니다.",
                    content = @Content(
                            mediaType = "application/json",
                            schema = @Schema(implementation = ChatRoomListResponseWithCount.class)
                    )),
            @ApiResponse(responseCode = "400", description = """
                    [UNAUTHORIZED] : 접근 권한이 없습니다. <br>
                    """,
    ...
    public BasicResponse<ChatRoomListResponseWithCount> findChatRoomList() {
        Member member = authService.loadMember();
        ChatRoomListResponseWithCount chatRoomResponse = chatRoomFacade.findChatRoomList(member);
        return ResponseUtil.success(chatRoomResponse);
    }
}